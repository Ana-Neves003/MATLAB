...
% Verifique se os sinais têm o mesmo comprimento
%if length(ecg_signal1) ~= length(ecg_signal2)
    %error('Os sinais ECG devem ter o mesmo comprimento.');
%end
...
% Configuração do subplot para comparar os histogramas dos 5 sinais
figure;

for i = 1:size(amostras_ecg,2)
    subplot(5, 5, i); % Ajuste conforme necessário
    histogram(amostras_ecg(:, i), nbins);
    title(['Histograma do Sinal ECG ', num2str(i)]);
    xlabel('Valores do Sinal');
    ylabel('Frequência');
end
