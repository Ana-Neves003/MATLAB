...
% Calcular a matriz de autocorrelação
%autocorrelacao_matriz_amostra = xcorr(todas_amostras, 'coeff');


% Calcula a matriz de autocorrelação usando apenas o somatório dos produtos
%matriz_autocorrelacao = zeros(size(todas_as_amostras - 40, 1));

for i = 1:size(todas_as_amostras - 40, 2)
    for j = 1:size(todas_as_amostras, 2)
        matriz_autocorrelacao(i, j) = sum(todas_as_amostras(:, i) .* todas_as_amostras(:, j));
    end
end

% Exibe o resultado
disp('Matriz de Autocorrelação usando Somatório dos Produtos:');
disp(matriz_autocorrelacao);
