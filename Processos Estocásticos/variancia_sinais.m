...
% Calcular a variância para cada sinal individual
variancia_por_sinal = var(amostras_ecg);

% Calcula a variância considerando todos os valores em todas as colunas
variancia_total = var(amostras_ecg(:));

% Plot as variâncias para cada sinal
figure;
subplot(1, 1, 1);
bar(variancia_por_sinal);
title('Variância para cada sinal');
xlabel('Sinal');
ylabel('Variância');

% Plot a variância total
%subplot(2, 1, 2);
%bar(variancia_total);
%title('Variância total de todos os sinais');
%xlabel('Sinal');
%ylabel('Variância Total');

% Ajusta o layout da figura
%sgtitle('Variação entre Sinais ECG');
